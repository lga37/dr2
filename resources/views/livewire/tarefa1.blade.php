<div>
    <x-slot name="header">
        ...
        <p>A Tarefa 1 tem como foco principal a análise comparativa da toxicidade nos comentários raiz de vídeos do YouTube. 
            O exercício consiste em buscar vídeos relacionados a termos potencialmente polarizadores — como "aborto", por exemplo 
            — e, a partir da seleção de vídeos exibidos, extrair seus comentários públicos e aplicar uma análise automática de 
            toxicidade (API Perspective). O objetivo é comparar a média de toxicidade entre os vídeos retornados pela mesma busca, 
        ...
        <div class="mx-auto max-w-12xl sm:px-6 lg:px-8">
                ...
                <div class="p-6 overflow-hidden overflow-x-auto bg-white border-b border-gray-200">
                    ...
                    <table class="min-w-full text-sm border divide-y divide-gray-300 table-auto">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                            ...
                            <tr class="">
                                <td class="px-2 py-1">
                                    <input type="checkbox" wire:model="selecionados" value="{{ $busca['videoId'] }}">


                                <td class="px-2 py-1 text-sm">{{ $busca['videoId'] }}</td>
                                <td class="px-2 py-1 text-sm">
                                    <a href="https://www.youtube.com/watch?v={{ $busca['videoId'] }}" class="text-blue-600 font-semibold" target="_blank">
                                        {{ $busca['title'] }}
                                    </a>
                                </td>
                                ...
                                <td class="px-2 py-1 text-sm">{{ \Carbon\Carbon::parse($busca['published'])->format('d/m/Y') }}</td>
                                <td class="px-2 py-1">
                            ...
                            @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-sm text-gray-500">Nenhum resultado encontrado.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <div class="mt-4">
                        ...
                        <div wire:loading wire:target="getRes" class="text-sm text-gray-500 mt-2">
            ...
            <div class="overflow-x-auto mt-8">
                <table class="
                    ...
                    table-auto min-w-full text-sm tracking-tight leading-tight
                    ">
                    <thead>
                        <tr class="bg-gray-100 text-xs text-gray-700 text-center">
                            ...
                            @endphp
                            <th colspan="5" style="width: {{ $colWidth * 5 }}%;" class="border border-gray-300 px-2 py-1">
                                <a href="https://www.youtube.com/watch?v={{ $video_id }}" target="_blank" class="text-blue-600 hover:underline">
                                    {{ $video_id }}
                                </a>
                            </th>
                            @endforeach
                    ...
                    <tbody>
                        ...
                        @for($i = 0; $i < $max; $i++)
                            ...
                            @if($c)
                            <td class="border border-gray-300 w-[10px] text-left text-gray-800 text-[10px] ">{{ $i + 1 }}</td>
                            <td class="border border-gray-300 max-w-[800px] text-sm whitespace-nowrap  overflow-hidden text-ellipsis">
                                {{ \Illuminate\Support\Str::words(strip_tags($c['text'] ?? '[sem texto]'), 20, '...') }}
                            </td>
                            <td class="border border-gray-300 w-[10px] text-gray-800 text-[10px]">{{ $c['likeCount'] ?? 0 }}</td>
                            <td class="border border-gray-300 w-[20px] text-gray-800 text-[10px]">{{ isset($c['publishedAt']) ? \Carbon\Carbon::parse($c['publishedAt'])->format('d/m/Y') : '--' }}</td>
                            <td class="border border-gray-300 w-[10px] text-gray-800 text-[10px]">
                                {{ isset($c['toxicity']) ? number_format($c['toxicity'] * 100, 1) . '%' : 'X' }}
                            </td>
                            @else
                            <td colspan="5" class="border border-gray-300 w-full text-gray-900 text-center italic text-[11px]">[sem comentário]</td>
                            @endif
                    ...
                    <tr class="bg-white">
                        ...
                        <td colspan="5" class="border border-gray-300 text-center">
                            <div class="relative w-full h-[{{ $maxHeight }}px] bg-gray-100 overflow-hidden">
                                ...
                                $color = $tox >= 0.7 ? 'bg-red-500' : ($tox >= 0.4 ? 'bg-yellow-400' : 'bg-green-400');
                                @endphp

                                @if($tox)
                                <div class="absolute bottom-0 {{ $color }}" style="left: {{ $left }}px; width: 1px; height: {{ $height }}px;" title="Tox: {{ number_format($tox*100,1) }}%"></div>
                                @endif
                                @endforeach
                            </div>
                        </td>
                    ...
                    <tfoot>
                        <tr class="bg-gray-50 border-t border-gray-300 text-[11px] text-gray-700 font-semibold text-center">
                            @foreach($comentarios as $video_id => $dados)
                            @php
